...
//  
//
//  Created by David Vallas on 4/19/20.
//
...
extension String {
    ...
    var innerBracket: String {
        let last = self.components(separatedBy: "<").last ?? self
        let first = last.components(separatedBy: ">").first ?? self
        return first
    }
