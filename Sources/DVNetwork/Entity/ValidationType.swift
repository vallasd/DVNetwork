...
//    case network
    ...
    func validate(any: Any, entityData: EntityData) throws {
        do {
            ...
            let e = error as! DVError
            throw e.update(header: .validation(entityData.name))
        }
        
    }
    
    private func validateJSON(json: JSON, entityData: EntityData) throws {
        guard let dict = json as? JSONDict else { throw DVError.notAny(key: entityData.name, not: JSONDict()) }
            let pd = entityData.dict
            for key in pd.keys {
                let j = dict[key]
                let data = pd[key]!
                if !data.optional {
                    if j == nil { throw DVError.requiredIsNil(key: key) }
    ...
    private func validateJSON(value: JSON, primitiveData: PrimitiveData, key: String) throws {
        ...
        if ct == .array || ct == .set {
            guard let array = value as? JSONArray else {
                throw DVError.notAny(key: key, not: JSONArray.self)
            }
            for value in array {
    ...
    private func validateJSON(value: JSON, primitive: Primitive, key: String) throws {
        ...
        case .int, .int16, .int32, .int64, .Enum:
            guard let i = value as? Int else { throw DVError.notAny(key: key, not: primitive) }
            switch primitive {
        ...
        if !success {
            throw DVError.notAny(key: key, not: primitive)
        }
    }
